...
export function DataFreshness({ lastUpdated, inline }: DataFreshnessProps) {
  const { mockDataEnabled } = useMockData();

  const content = (
    <div className="flex flex-wrap items-center gap-x-[var(--space-md)] gap-y-[2px] text-[length:var(--text-caption)] text-text-secondary">
      {lastUpdated && (
        <>
  ...
  return (
    <div className="border-b border-border-default bg-surface/60 -mx-[var(--space-base)] sm:-mx-[var(--space-lg)] px-[var(--space-base)] sm:px-[var(--space-lg)] py-[var(--space-xs)] mb-[var(--space-lg)]">
      {content}
    </div>
  );
}
